...
class LarsState(object):
    ...
    def __init__(self, X, y, max_features, verbose=False, X_test=None):
        ...
        assert self.signCov.shape == (
            n_features,
        )  # TODO: maybe signCov can be eliminated all together
        self.C = np.max(np.fabs(self.Cov))  # covariance of the active covariates
        self.CData = np.inf
        self.prevCData = np.inf

        self.L = np.zeros(
            (max(int(0.1 * max_features), 1), max(int(0.1 * max_features), 1)),
            dtype=y.dtype,
    ...
    def check_condition_threshold(self):
        ...
        assert (
            est_cond_triangular(self.L[:n_active, :n_active]) ** 2
    ...
    def add_index(self):
        ...
        if len(self.active) == self.max_features:
            raise ConvergenceWarning(
                "Stopping the lars path, as every regressor is active."
            )
        ...
        while True:
            ...
            if n_active > 0:
                ...
                # EU = np.linalg.norm(self.G[:n_active, self.active], ord=2)
                ER = np.sum(abs(R))
                diag = self.G[np.arange(len(self.active)), self.active]
            ...
            if idcs_rem == [
                C_idx
            ]:  # The last added regressor causes the system to become ill-conditioned.
    ...
    def remove_ill_conditioned_indices(self):
        ...
        if len(idcs_rem) > 0:
            ...
            rel_eigs_rem = (
                "{"
                + ",".join(
                    f"{idx}: {eig/max_eig:.2e}" for idx, eig in zip(idcs_rem, eigs_rem)
                )
                + "}"
            )
            warnings.warn(
    ...
    def step(self):
        ...
        n_active = len(self.active)
        least_squares, _ = self.solve_cholesky(
            self.L[:n_active, :n_active], self.signCov[self.active], lower=True
        )
        ...
        corr_eq_dir = (
            least_squares @ self.G[:n_active, self.inactive]
        )  # a as defined in (2.12)

        gamma_hat = min(
            min_pos(
                (self.C - self.Cov[self.inactive]) / (AA - corr_eq_dir + self.tiny)
            ),
            min_pos(
                (self.C + self.Cov[self.inactive]) / (AA + corr_eq_dir + self.tiny)
            ),
...
def lasso_lars(X, y, alpha, max_iter=500, verbose=False, X_test=None):
    ...
    for n_iter in range(max_iter):
        if n_iter > 0 and this_alpha <= alpha:
            ...
            coef = prev_coef + (prev_alpha - alpha) / (prev_alpha - this_alpha) * (
                coef - prev_coef
            )
            this_alpha = alpha
            break
        ...
        if prev_alpha == this_alpha:
            warnings.warn(
                "Early stopping the lars path, as correlations do not decrease.",
                ConvergenceWarning,
            )
            break
        assert (
            prev_alpha > this_alpha
        )  # This should be guaranteed by the way lars.C is updated.
    else:
...
def lasso_lars_cv(
    ...
    class GramedOperator(object):
        ...
        def XTX(self, _index):
            ...
            if self._gramian.shape[0] <= n_active:
                ...
                self._gramian = np.pad(
                    self._gramian,
                    ((0, new_capacity - old_capacity), (0, 0)),
                    mode="empty",
                )
    ...
    def res(fold):
        ...
        tmp = X.take_rows(slc(fold)).error_norm(
            y[slc(fold)], lars[fold].active, lars[fold].coef
        )
        ...
        if X_test is None:
            ...
            lars.append(
                LarsState(
                    ...
                    max_features,
                    verbose=verbose,
                    X_test=X_test.drop_rows(slc(fold)),
                )
            )
    ...
    for n_iter in range(max_iter):
        ...
        not_falling[idx] = (
            np.min(residuals[idx][-overtrainingSteps:])
            > overtrainingFactor * minimum[idx]
        )
    ...
    all_residuals = [
        np.interp(all_alphas, a[::-1], r[::-1]) for a, r in zip(alphas, residuals)
    ]
    ...
    ret = lasso_lars(
        X, y, best_alpha, max_iter=max_iter // cv, verbose=verbose, X_test=X_test
    )
    ret.n_iter_ = n_iter
